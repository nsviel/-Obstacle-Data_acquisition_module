...
def start_l1_capture():
    l1_device = param_capture.state_capture["lidar_1"]["device"]
    l1_port = param_capture.state_capture["self"]["l1_port"]
    ...
    # Start captue
    connected = param_capture.state_capture["lidar_1"]["connected"]
    if(connected and device_ok):
        ...
        while param_capture.run_thread_l1 and param_capture.state_capture["lidar_1"]["connected"]:
            if(param_capture.state_capture["lidar_1"]["activated"]):
                (header, packet) = listener.next()
                if(packet != None):
                    param_capture.sock_client.sendto(packet, (ip, port))
                    if(len(packet) == 1248):
                        param_capture.state_capture["lidar_1"]["packet"]["value"] += 1
                        terminal.addCstLog("cap", "LiDAR 2: [\033[1;32m%s\033[0m] packets"% param_capture.state_capture["lidar_2"]["packet"]["value"])
        terminal.addDaemon("#", "OFF", "LiDAR 1 capture")

def start_l2_capture():
    l2_device = param_capture.state_capture["lidar_2"]["device"]
    l2_port = param_capture.state_capture["self"]["l2_port"]
    ...
    # Start capture
    connected = param_capture.state_capture["lidar_2"]["connected"]
    if(connected and device_ok):
        ...
        while param_capture.run_thread_l2 and param_capture.state_capture["lidar_2"]["connected"]:
            if(param_capture.state_capture["lidar_2"]["activated"]):
                (header, packet) = listener.next()
                if(packet != None):
                    param_capture.sock_client.sendto(packet, (ip, port))
                    if(len(packet) == 1248):
                        param_capture.state_capture["lidar_2"]["packet"]["value"] += 1
                        terminal.addCstLog("cap", "LiDAR 2: [\033[1;32m%s\033[0m] packets"% param_capture.state_capture["lidar_2"]["packet"]["value"])
