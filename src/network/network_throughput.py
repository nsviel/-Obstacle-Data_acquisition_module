...
def start_daemon():
    try:
        thread_l1 = threading.Thread(target = thread_network_l1)
        thread_l2 = threading.Thread(target = thread_network_l2)
        thread_l1.start()
        thread_l2.start()
    ...
    while param_capture.run_thread_perf:
        try:
            if(param_capture.state_capture["lidar_1"]["connected"] and param_capture.state_capture["lidar_1"]["activated"]):
                l1_mbs = network_device(param_capture.state_capture["lidar_1"]["device"])
                ...
                param_capture.state_capture["lidar_1"]["throughput"]["max"] = tgp_max
            else:
            ...
            time.sleep(1)
        time.sleep(param_capture.tic_throughput)

def thread_network_l2():
    tgp_min = 1000
    tgp_max = 0
    tgp_list = [0]
    param_capture.run_thread_perf = True
    while param_capture.run_thread_perf:
        try:
            if(param_capture.state_capture["lidar_2"]["connected"] and param_capture.state_capture["lidar_2"]["activated"]):
                l2_mbs = network_device(param_capture.state_capture["lidar_2"]["device"])
